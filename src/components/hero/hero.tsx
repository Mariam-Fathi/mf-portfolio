...
const Hero: React.FC<HeroProps> = ({ onNavigate, onReady, isActive = true }) => {
        ...
        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
            ...
            if (pos) {
              ...
              finalDot.style.backgroundColor = "#6A0610";
              ...
              gsap.set(finalDot, {
                ...
                if (svgIRef.current) {
                  gsap.set(svgIRef.current, { fill: "#6A0610" });
                }
                if (svgA2Ref.current) {
                  gsap.set(svgA2Ref.current, { fill: "#6A0610" });
                }
                if (svgM2Ref.current) {
                  gsap.set(svgM2Ref.current, { fill: "#6A0610" });
                }
  ...
  const buildDotTimeline = () => {
    ...
    finalDotTimeline.to(finalDot, {
      keyframes: [
        { x: m2ScreenX - (finalDotSize / 2), y: m2ScreenY - 100, scaleY: 0.75, backgroundColor: "#6A0610", duration: 0.3, ease: "power2.out", force3D: true },
        { y: m2ScreenY - 120, duration: 0.2, ease: "sine.inOut", force3D: true },
        { y: m2ScreenY + 20, scaleY: 1.2, backgroundColor: "#6A0610", duration: 0.25, ease: "power2.in", force3D: true },
        {
          y: m2ScreenY,
          scaleY: 0.8,
          backgroundColor: "#6A0610",
      ...
      const timeoutId = setTimeout(() => {
        if (svgMariamTextRef.current && numberSevenRef.current && 
          ...
          if (dotTimeline && memoizedPositions) {
            ...
            if (svgIRef.current) gsap.set(svgIRef.current, { fill: "#6A0610" });
            if (svgA2Ref.current) gsap.set(svgA2Ref.current, { fill: "#6A0610" });
            if (svgM2Ref.current) gsap.set(svgM2Ref.current, { fill: "#6A0610" });
    ...
    const positionTextsOnM = (mariamTextElement: SVGTextElement, svg: SVGSVGElement, mariamFontSize: number) => {
      try {
        ...
        if (intoText) {
          ...
          intoText.setAttribute("opacity", "0");
          intoText.setAttribute("fill", "#6A0610");
          intoText.style.filter = "blur(10px)";
        ...
        if (ideasText) {
          ...
          ideasText.setAttribute("opacity", "0");
          ideasText.setAttribute("fill", "#6A0610");
          ideasText.style.filter = "blur(10px)";
          // No rotation - keep it horizontal
        ...
        if (turningIdeas) {
          ...
          turningIdeas.setAttribute("opacity", "0");
          turningIdeas.setAttribute("fill", "#6A0610");
          turningIdeas.style.filter = "blur(10px)";
        }
        ...
        if (realLifeProducts) {
          ...
          realLifeProducts.setAttribute("opacity", "0");
          realLifeProducts.setAttribute("fill", "#6A0610");
          realLifeProducts.style.filter = "blur(10px)";
        }
